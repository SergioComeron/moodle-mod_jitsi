...
$string['actaapikeyactivity_help'] = 'Optional OpenAI-compatible API key for this activity. If set, it is used instead of the site key. Leave empty to keep the stored key or inherit the site key. Without a key, session minutes stay off and the live meeting works as usual.';
$string['actaapikeyex'] = 'Your own OpenAI-compatible API key. The plugin never uses a vendor-paid key. If this field is empty and the activity has no key either, session minutes stay off. The key is never written to logs.';
...
$string['actaenabledex'] = 'When enabled and a BYOK API key is saved (here or on the activity), hanging up a session queues session minutes: a short summary, attendance from the existing participating logs, and pending follow-ups. Without a key the live meeting is unchanged. Recordings are not uploaded; a stored transcript is used when one already exists.';
...
$string['actaheadingex'] = 'Bring-your-own-key session minutes. Independent from Vertex AI recording tools. No billing, marketplace or vendor key is used.';
...
$string['actanotavailable'] = 'Session minutes are off because no LLM API key is configured.';
$string['actaprivacynote'] = 'Minutes use existing attendance logs. If a transcript already exists it is sent to your LLM; recordings are not uploaded.';
...
$string['privacy:metadata:actallm'] = 'When session minutes are enabled and a BYOK API key is configured, a session transcript (if one already exists) and attendance display names are sent to the configured OpenAI-compatible endpoint. Recordings are not uploaded. The site or activity owner supplies the key; no vendor-paid key is used.';
